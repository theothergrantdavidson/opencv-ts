...
type HOUGH_STANDARD = 0;
type HOUGH_PROBABILISTIC = 1;
type HOUGH_MULTI_SCALE = 2;
type HOUGH_GRADIENT = 3;
type HOUGH_GRADIENT_ALT = 4;

export type HoughModes =
    | HOUGH_STANDARD
    | HOUGH_PROBABILISTIC
    | HOUGH_MULTI_SCALE
    | HOUGH_GRADIENT
...
export interface IHoughModes {
    HOUGH_STANDARD: HOUGH_STANDARD;
    HOUGH_PROBABILISTIC: HOUGH_PROBABILISTIC;
    HOUGH_MULTI_SCALE: HOUGH_MULTI_SCALE;
    HOUGH_GRADIENT: HOUGH_GRADIENT;
    HOUGH_GRADIENT_ALT: HOUGH_GRADIENT_ALT;
}

type LSD_REFINE_NONE = 0;
type LSD_REFINE_STD = 1;
type LSD_REFINE_ADV = 2;

export type LineSegmentDetectorModes = LSD_REFINE_NONE | LSD_REFINE_STD | LSD_REFINE_ADV;

export interface ILineSegmentDetectorModes {
    LSD_REFINE_NONE: LSD_REFINE_NONE;
    LSD_REFINE_STD: LSD_REFINE_STD;
    LSD_REFINE_ADV: LSD_REFINE_ADV;
}
...
export interface FeatureDetection {
     ...
     * @param method Detection method, see  The available methods are HOUGH_GRADIENT and HOUGH_GRADIENT_ALT.
