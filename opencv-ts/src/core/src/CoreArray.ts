...
import { CovarFlags } from '../Core';

type DECOMP_LU = 0;
type DECOMP_SVD = 1;
type DECOMP_EIG = 2;
type DECOMP_CHOLESKY = 3;
type DECOMP_QR = 4;
type DECOMP_NORMAL = 16;
...
type BORDER_CONSTANT = 0;
type BORDER_REPLICATE = 1;
type BORDER_REFLECT = 2;
type BORDER_WRAP = 3;
type BORDER_REFLECT_101 = 4;
type BORDER_TRANSPARENT = 5;
type BORDER_REFLECT101 = BORDER_REFLECT_101;
type BORDER_DEFAULT = BORDER_REFLECT_101;
type BORDER_ISOLATED = 16;
...
export type CmpTypes = CMP_EQ | CMP_GT | CMP_GE | CMP_LT | CMP_LE | CMP_NE;

export interface ICmpTypes {
    CMP_EQ: CMP_EQ;
    CMP_GT: CMP_GT;
    CMP_GE: CMP_GE;
    CMP_LT: CMP_LT;
...
type DFT_SCALE = 2;
type DFT_ROWS = 4;
type DFT_COMPLEX_OUTPUT = 16;
type DFT_REAL_OUTPUT = 32;
type DFT_COMPLEX_INPUT = 64;
type DCT_INVERSE = DFT_INVERSE;
type DCT_ROWS = DFT_ROWS;
...
export type GemmFlags = GEMM_1_T | GEMM_2_T | GEMM_3_T;

export interface IGemmFlags {
    GEMM_1_T: GEMM_1_T;
    GEMM_2_T: GEMM_2_T;
    GEMM_3_T: GEMM_3_T;
}
...
type NORM_L2SQR = 5;
type NORM_HAMMING = 6;
type NORM_HAMMING2 = 7;
type NORM_TYPE_MASK = 7;
type NORM_RELATIVE = 8;
type NORM_MINMAX = 32;
...
export type ReduceTypes = REDUCE_SUM | REDUCE_AVG | REDUCE_MAX | REDUCE_MIN;

export interface IReduceTypes {
...
type ROTATE_90_CLOCKWISE = 0;
type ROTATE_180 = 1;
type ROTATE_90_COUNTERCLOCKWISE = 2;

export type RotateFlags = ROTATE_90_CLOCKWISE | ROTATE_180 | ROTATE_90_COUNTERCLOCKWISE;

export interface IRotateFlags {
    ROTATE_90_CLOCKWISE: ROTATE_90_CLOCKWISE;
    ROTATE_180: ROTATE_180;
    ROTATE_90_COUNTERCLOCKWISE: ROTATE_90_COUNTERCLOCKWISE;
}

export interface CoreArray {
    /**
     ...
     */
    findNonZero(src: Mat, idx: Mat | Array<Point>): void;
    /**
